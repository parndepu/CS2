...
void AST::mainHeader(const std::vector<std::string>& profileName) {
    ...
    for (std::vector<std::string>::const_iterator it = profileName.begin(); it < profileName.end(); ++it) {
        std::string filename = it->substr(0, it->find("_cpp"));
        profileString = profileString + "profile " + *it + "(\"" + filename + ".cpp\");\n";
    }
    profileString += "\n";
    
    // Add all profiler string before index
...
void AST::funcCount(const std::string& profileName) {
    ...
    for (std::list<AST*>::iterator index = child.begin(); index != child.end(); ++index) {
        
         if ((*index)->tag == "function" || (*index)->tag == "constructor" || (*index)->tag == "destructor") {
            
            std::string functionName = "";
            std::list<AST*>::iterator startItr = getChild((*index)->tag)->child.begin();
            std::list<AST*>::iterator endItr = getChild((*index)->tag)->child.end();
            
            for (std::list<AST*>::iterator it = startItr; it != endItr; ++it) {
                std::cout << (*it)->tag << std::endl;
                std::string functionName = "";
                
                if ((*it)->tag == "name") { functionName = (*it)->getName(); }
                if ((*it)->tag == "block") {
                    std::string profileString = "{" + profileName + ".count(__LINE__,\"" + functionName + "\");\n";
                    child.insert(it, new AST(whitespace, profileString));
                }
            }

            // TODO: need to skip line
        }
    }    
}
